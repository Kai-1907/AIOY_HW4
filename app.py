...
import google.generativeai as genai  # 改用最穩定的舊版 SDK 結構

# ================= 1. 配置 Gemini AI =================
# 直接從 Secrets 讀取 Key 並進行設定
...
def generate_food_report(food_name):
    try:
        ...
        # 如果 1.5-flash 還是不行，自動降級嘗試 gemini-pro
        try:
            model_backup = genai.GenerativeModel('gemini-pro')
            response = model_backup.generate_content(prompt)
            return response.text
        except:
            return f"AI 報告生成失敗，請檢查 API Key 權限。錯誤訊息：{str(e)}"
...
# ================= 3. Streamlit 介面設計 =================
st.title("🍔 食物辨識智能 Agent (穩定版)")
st.write("上傳食物照片，由深度學習模型辨識後，交給 AI 撰寫評論。")
